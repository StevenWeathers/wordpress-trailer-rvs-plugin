<?php
/**
 * Adds Trailer_By_Stock widget.
 */
class Trailer_By_Stock extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'trailer_by_stock', // Base ID
			__('Find by Stock Number', 'text_domain'), // Name
			array( 'description' => __( 'A form to Find Trailer by Stock Number', 'text_domain' ), ) // Args
		);
	}
    ...
    if (!empty($_GET)){
      $filtered_trailer_stock_num = $_GET['stock-number'];
    }
        ...
        echo '<form action="'.$filter_form_url.'" method="get" role="form">';
          echo '<div class="row">';
            echo '<div class="col-md-8">';
              if ( isset($filtered_trailer_stock_num) && !empty($filtered_trailer_stock_num)) {
                echo '<input type="text" name="stock-number" value="'.$filtered_trailer_stock_num.'" class="form-control" placeholder="Stock Number..."/>';
              } else {
