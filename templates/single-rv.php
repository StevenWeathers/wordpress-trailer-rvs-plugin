  ...
  <div class="trailer-detail" itemscope itemtype="http://schema.org/Product">
    ...
    <?php if ( get_field("ids_sold_date") && strpos(strtolower(get_field( "ids_status" )),'sold') !== false ) : ?>
            <div class="alert alert-success alert-status-sold">
              <p>
                    <strong>This RV was sold on <?php echo date('F j, Y', strtotime(get_field("ids_sold_date"))); ?></strong><br />
                    If you are interested in a RV like this one feel free to use the form below to contact us or browse our site.
                </p>
            </div>
        <?php elseif( strpos(strtolower(get_field( "ids_status" )),'sold') !== false ) : ?>
          <div class="alert alert-success alert-status-sold">
              <p>
                    <strong>This RV was sold</strong><br />
                    If you are interested in a RV like this one feel free to use the form below to contact us or browse our site.
                </p>
            </div>
        <?php endif; ?>
                ...
                <div id="detail-photos">
                  <?php
                    ...
                    if (!empty($image_ids)) {
                      ...
                      $shortcode = '[soliloquy_dynamic id="custom-rv-detail" images="'.implode(',', $image_ids).'"]';
            ...
            <ul class="list-unstyled">
              ...
              <li class="row" style="border-bottom: 1px solid #eaeaea; line-height: 28px;">
                ...
                <div class="col-md-8 span8" itemprop="manufacturer" itemscope itemtype="http://schema.org/Organization">
                      <span itemprop="name"><?php echo get_the_term_list( $post->ID, 'rv-brand', '', ', ', '' ); ?></span>
                </div>
              </li>
              <li class="row" style="border-bottom: 1px solid #eaeaea; line-height: 28px;">
              ...
              <li class="row" style="border-bottom: 1px solid #eaeaea; line-height: 28px;">
                ...
                <div class="col-md-8 span8"><?php echo get_the_term_list( $post->ID, 'rv-type', '', ', ', '' ); ?></div>
              </li>
                <?php
    ...
    <div class="row">
        ...
        <div class="col-md-6 span6">
            <div class="l-inquire">
              <h3>QUESTIONS ABOUT THIS RV?</h3>
                <?php
                  $rv_import_display_options = get_option( 'rv_import_display' );
                  gravity_form($rv_import_display_options['rv_inquire_form_id'], false, false, false, array( 'stock_number' => get_field( "ids_stock_number" ), 'rv_title' => get_the_title($post->ID), 'rv_brand' => get_the_term_list( $post->ID, 'rv-brand', '', ', ', '' ), 'rv_model' => get_field('ids_model'), 'rv_year' => get_field('ids_year') ), true, 12);
                ?>
            </div>
        </div>
    </div>
    <div class="row">
      <?php
        $trailer_disclaimer = of_get_option('rvs_detail_disclaimer', '');
        if ( $trailer_disclaimer ) {
          echo '<div class="trailer-disclaimers col-xs-12 col-sm-12 col-md-12" style="margin-top: 40px; color: #666">' . of_get_option('rvs_detail_disclaimer') . '</div>';
        }
      ?>
    </div>
  </div>
